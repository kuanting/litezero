...
| 16 | 7 concurrent AES-GCM streams sustainable on the target board without back-pressure | Lab measurement (the multi-stream table appeared in the pre-revision system paper and was removed in the 2026-09 major revision as peripheral to the security contribution) | Empirical; no datasheet constraint. Consistent with UG1085 §12.4.1 which lists one AES-GCM engine and suggests interleaving. | **CONFIRMED (measurement)** — no longer reported in either paper |
| 17 | d_D resides in PS DRAM only for the unwrap–multiply–zeroize sequence, bounded by the measured ≈2.1 ms drone-side handshake stage; `mlock()` pinning and XMPU page isolation are platform-supported hardenings, not configured in the prototype | Secure_IoT_FPGA_Drone Sec 7.1 | Architectural (immediate zeroization traced in `src/services/drone.ts`), bounded by the measured handshake-stage timing; XAPP1333 §2.2 permits the one-aperture-per-master configuration for a production build. | **REVISED** — a direct instrumented measurement of the residency window, and the mlock/XMPU configuration, are future work |
...
- Not a formal security proof. That lives in the LiteZero paper Sec V and the four Verifpal models under `models/` (Verifpal 0.53.0).
